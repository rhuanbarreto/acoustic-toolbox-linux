function NL = modal_noise_diag( ModeFile, rho_SL_dB, sd, rd, Rmax_km, Component )
...
% Also, the assumption that off-diagonal terms can be dropped is not always
% valid
...
for ir = 1 : length( Rmax )
   ...
   for mode = ii
      ...
      ki   = imag( Modes.k( mode ) );
      kabs = abs(  Modes.k( mode ) );
...
NL = 10 * log10( NL );   % convert to dB
    ...
    NL = NaN;   % return a NaN is there are no modes
end
