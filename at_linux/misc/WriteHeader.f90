SUBROUTINE WriteHeader( FileName, Title, Atten, PlotType )
  ...
  IF ( PlotType( 1 : 2 ) /= 'TL' ) THEN
     ...
     WRITE( SHDFile, REC = 5 ) Pos%theta
     WRITE( SHDFile, REC = 6 ) Pos%sx
     WRITE( SHDFile, REC = 7 ) Pos%sy
     WRITE( SHDFile, REC = 8 ) Pos%sd
     WRITE( SHDFile, REC = 9 ) Pos%rd
     WRITE( SHDFile, REC = 10 ) Pos%r
