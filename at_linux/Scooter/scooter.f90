PROGRAM SCOOTER
  ...
  FreqLoop: DO ifreq = 1, Nfreq
     ...
     CALL CalculateKernel( NPoints, k, Green, NTotal1 )
...
SUBROUTINE CalculateKernel( NPoints, k, Green, NTotal1 )
  ...
  SourceDepth: DO is = 1, Pos%Nsd
     RcvrDepth: DO ir = 1, Pos%Nrd
        WRITE( GRNFile, REC = 10 + ( is - 1 ) * Pos%Nrd + ir ) Green( is, ir, : )
     END DO RcvrDepth
  END DO SourceDepth
