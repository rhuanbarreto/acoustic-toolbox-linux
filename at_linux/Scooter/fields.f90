PROGRAM FIELDS
  ...
  ! Note: The Matlab version of this (fieldsco.m) is preferred
  ...
  IF ( kmax < k( Nk ) ) THEN
     ...
     write( PRTfile, * ) 'IRatioDelta_r', IRatioDelta_r
...
SUBROUTINE ReadHeader
  ...
  READ( GRNFile, REC = 3 ) Nfreq, Pos%Ntheta, Pos%Nsx, Pos%Nsy, Pos%Nsd, Pos%Nrd, Nk, atten

  ALLOCATE( freqVec( 1 : Nfreq ), Pos%sd( Pos%Nsd ), Pos%rd( Pos%Nrd ), k( Nk ), G( Nk ), Stat = IAllocStat )
  IF ( IAllocStat /= 0 ) CALL ERROUT( PRTFile, 'F', 'RDHEAD', 'Too many source/receiver combinations' )

  READ( GRNFile, REC = 4 ) freqVec( 1 : Nfreq )
  READ( GRNFile, REC = 8 ) Pos%sd
  READ( GRNFile, REC = 9 ) Pos%rd
  READ( GRNFile, REC = 10 ) k(  1 : Nk  )  ! vector of k-space points
  Delta_k = k( 2 ) - k( 1 )
