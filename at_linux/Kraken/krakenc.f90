PROGRAM KRAKENC
  ...
  Profile: DO iProf = 1, 9999   ! Loop over a sequence of profiles
     ...
     freq0 = freqVec( 1 )   ! save the reference frequency for scaling the grid
     ...
     FreqLoop: DO ifreq = 1, Nfreq
        ...
        DO ISet = 1, NSets   ! Main loop: solve the problem for a sequence of meshes
           N( 1 : SSP%NMedia ) = NG( 1 : SSP%NMedia ) * NV( ISet ) * freqVec( ifreq ) / freq0   ! scaled by frequency
...
3000    omega   = SQRT( omega2 )   ! Solution complete: discard modes with phase velocity above cHigh
        ...
        DO IREC = 1, 1 + ( 2 * M - 1 ) / LRecordLength
           ...
           WRITE( MODFile, REC = IRecProfile + M + IREC ) CMPLX( k( IFirst : ILast ) )
           IFirst = ILast + 1
        END DO

        ! set record pointer to beginning of next mode set
        IRecProfile = IRecProfile + 1 + M + 1 + ( 2 * M - 1 ) / LRecordLength
     END DO FreqLoop
  END DO Profile
...
SUBROUTINE Solve2( FileRoot )
  ...
  IMPLICIT NONE
  INTEGER            :: ITry, MaxTries, NzTab, IFirst, ILast, IRec
  INTEGER            :: ii, J
  INTEGER            :: Iteration, MaxIteration = 2000   ! iteerations in root finderREAL               :: RVAR1, RVAR2, kLow, kHigh
...
SUBROUTINE Vector( FileRoot, NTotal, NTotal1 )
  ...
  IF ( ifreq == 1 ) THEN
     ...
     WRITE( MODFile, REC = IRecProfile + 4 ) freqVec( 1 : Nfreq )
  ...
  ModeLoop: DO mode = 1, M
     ...
     ELSE
        ...
        WRITE( MODFile, REC = IRecProfile + mode ) PhiTab
     END IF
  END DO ModeLoop
