...
SUBROUTINE NewProfile( FileRoot, iProf, ifreq, k, phiR, MR, rd, Nrd, A ) 
  ...
  COMPLEX              :: gamTR,         gamBR,         phiTR = 0,     phiBR = 0, KTop2R, KBot2R, tail
  ...
  ModeLoop: DO mode = 1, MR
     READ( ModeFile, REC = IRecProfile + mode ) ( phi( iz ), iz = 1, Nr )   ! read in the mode
...
SUBROUTINE PLeft( FileRoot, iProf, ifreq, IRecProfile, A, k, z, M, P, NR, NTot,           &
  ...
  INTEGER              :: N( Maxmed ), NL, NMat, med, mode, NMedia, iz, izL, Nfreq, ifreqT, LRecl, MR
  INTEGER, SAVE        :: IRecProfileL = 1, IRecProfileR = 1
  ...
  CALL ReadModeHeader( FileRoot, iProf - 1, IRecProfile, LRecl, Title, freqVec, Nfreq, Nmedia, NL, NMat, N, Material, &
       depthL, rhoL, &
       BCTop, rhoT, depthTL, &
       BCBot, rhoB, depthBL, zL, cPT, csT, cPB, csB )
  ...
  DO mode = 1, MIN( ML, M )  ! There are only M mode coefficients we can use, if ML happens to be larger
     READ( ModeFile, REC = IRecProfile + mode ) phi
  ...
  IRecProfileR = IRecProfile + ML + 2 + ( 2 * ML - 1 ) / LRecL

  ! Read modal data in new segment
  IRecProfile = IRecProfileR

  CALL ReadModeHeader( FileRoot, iProf, IRecProfile, LRecl, Title, freqVec, Nfreq, Nmedia, NR, NMat, N, Material, &
       depth, rho, &
       BCTop, rhoT, depthT, &
