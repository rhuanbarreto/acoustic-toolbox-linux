MODULE ReadModes
  ...
  INTEGER, PARAMETER          :: MaxN = 16001, MaxMedium = 51
...
CONTAINS
  ...
  ! The modefile starts with 7 descriptive hearder records
  ...
  INTEGER            :: Nfreq, iRec, IRecProfile = 1, LRecl, Mode, NMat, NMedia, NTot, N( MaxMedium ), ird( Nrd ) 
  ...
  CALL ReadModeHeader( FileRoot, IProf, IRecProfile, LRecl, Title, freqVec, Nfreq, NMedia, NTot, NMat, N, Material, Depth, rho, &
       BCTop, rhoT, DepthT, & 
       BCBot, rhoB, DepthB, Z, cPT, cST, cPB, cSB )
  ...
  IRecProfile = IRecProfile + ( M + 2 + ( 2 * M - 1 ) / LRecl )
...
SUBROUTINE ReadModeHeader( FileRoot, IProf, IRecProfile, LRecl, Title, freqVec, Nfreq, &
     NMedia, NTot, NMat, N, Material, Depth, rho,&
     BCTop, rhoT, DepthT, &
     BCBot, rhoB, DepthB, Z, cPT, csT, cPB, csB )
  ...
  REAL,               INTENT(OUT) :: Depth( * ), Z( * ), rho( * ), rhoT, DepthT, rhoB, DepthB
  REAL      (KIND=8), INTENT(OUT) :: freqVec( * )
  COMPLEX,            INTENT(OUT) :: cPT, cST, cPB, cSB
  CHARACTER (LEN= *), INTENT(OUT) :: Title
  CHARACTER (LEN= 8), INTENT(OUT) :: Material( * )
  CHARACTER (LEN= 1), INTENT(OUT) :: BCTop, BCBot
  LOGICAL              :: OpenFlag
  ...
  READ( ModeFile, REC = IRecProfile + 4 ) freqVec( 1 : Nfreq )
...
SUBROUTINE ReadWavenumbers( IRecProfile, ifreq, k, M, MaxM, LRecL )
  ...
  DO ifreqT = 1, ifreq
     IF ( ifreqT > 1 ) IRecProfile = IRecProfile + ( M + 2 + ( 2 * M - 1 ) / LRecl )   ! advance to appropriate record
  ...
  DO IRec = 1, 1 + ( 2 * MIN( M, MaxM ) - 1 ) / LRecL
     ...
     READ( ModeFile, REC = IRecProfile + M + IRec ) k( IFirst : ILast )      
     IFirst = ILast + 1
  END DO
...
SUBROUTINE ReadOneMode( Mode, IRecProfile, NTot, NMat, W, ird, N, Material, NMedia, Comp, &
  ...
  READ( ModeFile, REC = IRecProfile + Mode ) ( Phi( iMat ), iMat = 1, NMat ) 
